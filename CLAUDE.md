...
- **server.py** — FastMCP server instance, 3 tools (`generate_image`, `refine_image_prompt`, `list_generated_images`), and `_generate()` which calls litellm and extracts the image data URL from the response. `generate_image` supports optional `feedback` (regeneration) and `reference_images` (list of file paths read and base64-encoded for multimodal input).
